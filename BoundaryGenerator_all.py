...
def buildScaledAverage(genes, chromosomeReads):
    posAverageCurve = np.zeros(1000)
    negAverageCurve = np.zeros(1000)
    avgGeneStart = 0
    for gene in genes:
        avgGeneStart = avgGeneStart + 1000 * (gene.leftBoundary - gene.leftIntergenicBoundary)/(gene.rightIntergenicBoundary - gene.leftIntergenicBoundary)
        posReads = {}
        negReads = {}
        if (gene.chromosome + '1') in chromosomeReads:
            if gene.sense == 1:
            ...
            readCounter = float(gene.leftIntergenicBoundary)
            increment = (gene.rightIntergenicBoundary - gene.leftIntergenicBoundary) / 1000.0
            #for i in range(gene.leftIntergenicBoundary, gene.rightIntergenicBoundary + 1)
            for averageCounter in range(0,1000):
                if int(readCounter) in posReads  and int(readCounter) in negReads:
                    posAverageCurve[averageCounter] = posReads[int(readCounter)]
                    negAverageCurve[averageCounter] = negReads[int(readCounter)]
                    readCounter = readCounter + increment
    avgGeneStart = avgGeneStart/ len(genes)
    plt.plot(posAverageCurve, 'r')
    plt.plot(negAverageCurve, 'b')
    plt.axvline(x=avgGeneStart, color='k')
    plt.show()
